        ...
        for j in range(len(key2)):
            if list(key1.keys())[i] in list(key2.keys())[j]:
                ...
                if int(str(list(key1.values())[i])) > int(str(list(key2.values())[j])):
                    katsayi_benzerlik_orani += int(str(list(key2.values())[j])) / int(str(list(key1.values())[i])) * 100
                else:
                    katsayi_benzerlik_orani += int(str(list(key1.values())[i])) / int(str(list(key2.values())[j])) * 100
        if varmi is False:
            farkli_sayi += 1
